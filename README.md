...
skill 会自动:识别当前工程/SDK(子目录名由你确认)→ 探测内核 commit / 构建命令等 → 追问缺失的 env → 用模板写好 case 文件组 → 登记 index → 按工程类型(repo 多仓 / 普通单仓)给回链建议。`git commit` 默认本地做;**`git push` 必须每次显式同意**——不会在你不知情时把含硬件细节的 case 推上去。
