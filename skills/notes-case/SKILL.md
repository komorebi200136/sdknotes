...
4. **先给清单、再写盘**: 写文件前,用一小段告诉用户"**我从对话里提取到这些 → 准备写到哪**"(每条一行:内容摘要 + 目标文件/段 + 轮次号),让用户扫一眼纠错。拿不准是不是该记的(可能是临时尝试)就标出来问一句,别擅自当结论写进去。
5. 确认/默许后按上面的路由写入各文件,同步 index 现状行,`git commit`。
