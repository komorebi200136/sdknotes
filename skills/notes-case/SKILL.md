...
大段 dmesg / 串口 / ADB 日志 / 完整 diff / 截图 → 写进 case 目录的 `logs/`,**按轮次命名** `t<N>-<标识>-<类型>.txt`,并在 case.md 「附件」段引用。
...
2. **只更新相关段,不重写整个文件**:
   ...
   - 试过的命令/实验过程(过程性,带结果)→ trace 段,**用轮次号 `t<N>` 起头**
   - 参考链接(datasheet 页/内核文档/相似 commit/issue/社区帖)→ references 段
   - 中途待办/待验证假设/当前卡点 → next 段(多轮接续全靠它,下次开 agent 先看这里)
   - **持续抓到的日志(串口/ADB/dmesg)→ 关键步骤**:先确认它对应哪次改动(接着上一轮就 `t<N+1>`),按 `logs/t<N>-<标识>-<类型>.txt` 存,并在 trace 加/补一条 `t<N> 改了X → 结果Y → 日志 logs/t<N>-...`,做到**改动与日志一一对应**;原始大段日志只进 logs/,trace 里只留结论
