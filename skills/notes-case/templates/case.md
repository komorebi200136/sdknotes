...
> 创建: YYYY-MM-DD
>
> 关联文件: [调试过程 trace.md](trace.md) · [参考资料 references.md](references.md) · 原始日志见 `logs/`
...
## 目标 / 现象
> bug: 预期 vs 实际 + 复现概率(间歇性 bug 必写)+ 关键日志(原文进 logs/)。
> feature: 想实现什么 + 验收标准。

- 预期:
- 实际:
- 复现概率/触发条件:
- 关键日志: 见 [trace.md](trace.md) 各轮次 + `logs/`
...
## 排查 / 设计 (findings)
> 结论性的:排除链路 + 根因 + 关键变量/日志含义。详细的逐轮尝试记到 [trace.md](trace.md)。
