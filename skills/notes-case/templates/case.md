...
## 调试过程 / 实验 (trace)
> 过程性记录。每次"改动 → 烧录 → 抓日志"配成一条,用轮次号 `t<N>` 串起来:日志按 `t<N>` 存进 logs/,这里引用,做到**哪次改动对应哪个 log 可追溯**。
...
## 附件
> 日志/diff/截图放 logs/,**按轮次命名** `t<N>-<简短标识>-<类型>.txt`(如 `t2-reset10ms-dmesg.txt`),与 trace 的 `t<N>` 对应。
